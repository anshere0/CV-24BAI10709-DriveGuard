...
export async function cancelJob(jobId: string): Promise<AnalysisJobDto> {
  return requestJson<AnalysisJobDto>(`/analysis-jobs/${jobId}/cancel`, {
    method: "POST",
  });
}
...
export async function fetchSessionEdgeSummary(sessionId: string): Promise<SessionEdgeSummaryDto> {
  return requestJson<SessionEdgeSummaryDto>(`/sessions/${sessionId}/edge-summary`);
}
