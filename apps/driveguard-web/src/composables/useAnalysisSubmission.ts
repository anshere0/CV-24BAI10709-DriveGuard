...
import { cancelJob, createAnalysisJob, fetchJob, uploadVideo } from "../services/api";
import type { AnalysisJobDto, UploadedVideoDto } from "../types/backend";

interface UseAnalysisSubmissionOptions {
  configPath?: string;
  sourceOrigin?: "web_upload" | "web_live" | "android_upload";
...
export function useAnalysisSubmission(options?: UseAnalysisSubmissionOptions) {
  ...
  async function submitFile(file: File, labels?: { idle?: string; uploading?: string }): Promise<void> {
    ...
    try {
      ...
      const job = await createAnalysisJob({
        ...
        config_path: options?.configPath ?? "config.toml",
      });
  ...
  async function cancelCurrentJob(): Promise<void> {
    if (!createdJob.value || !canCancelJob.value) {
      return;
    }
    ...
    error.value = "";
    try {
      const job = await cancelJob(createdJob.value.id);
      createdJob.value = job;
      syncJobProgress(job);
      if (isTerminalStatus(job.status)) {
        progressMode.value = resolveTerminalMode(job.status);
      ...
      error.value = err instanceof Error ? err.message : "Failed to cancel analysis job.";
    } finally {
