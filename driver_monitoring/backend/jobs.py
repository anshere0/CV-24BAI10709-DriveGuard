...
from driver_monitoring.backend.models import AnalysisJob, ReportArtifact
...
from driver_monitoring.backend.services import (
    ...
    reconcile_edge_events_for_uploaded_video,
    reset_job_results,
    resolve_job_source_paths,
)
from driver_monitoring.pipeline import PipelineConfig
from driver_monitoring.runner import FrameAnalysis, run_headless
...
def request_job_cancel(job_id: str, config_path: str = "config.toml") -> bool:
    with session_scope(config_path) as session:
        ...
        if job.status == "queued":
            ...
            job.progress_message = "Analysis canceled before processing started."
            job.completed_at = datetime.now(timezone.utc)
        else:
            job.progress_phase = "canceling"
            job.progress_message = "Cancellation requested. Stopping analysis."
        session.commit()

    stop_event = _get_stop_event(job_id)
    if stop_event is not None:
        stop_event.set()
    return True


def process_analysis_job(job_id: str, config_path: str = "config.toml", stop_event: Optional[Event] = None) -> None:
    ...
    with session_scope(config_path) as session:
        ...
        try:
            ...
            for report in batch_report.session_reports:
                ...
                if source_path and source_path in uploaded_video_by_path:
                    reconcile_edge_events_for_uploaded_video(
                        session=session,
                        job=job,
                        analysis_session=persisted_session,
                        uploaded_video_id=uploaded_video_by_path[source_path],
                        report=report,
                    )
